...
const Home = () => {
  ...
  useEffect(() => {
    const checkAuth = async () => {
      ...
      try {
        const res = await axios.get("https://snapcap-your-buddy-for-captions.onrender.com/api/posts/", {
          withCredentials: true,
        });
        // Check for a specific response indicating successful authentication
        if (res.status === 200 && res.data?.message !== "Unauthorized") {
          setIsAuthenticated(true);
          console.log("Auth check:", res.data.message);
        } else {
          setIsAuthenticated(false);
          console.log("Not authenticated: Invalid response");
        }
      } catch (err) {
        setIsAuthenticated(false);
  ...
  const handleGenerateCaption = async () => {
    ...
    try {
      ...
      const res = await axios.post(
        "https://snapcap-your-buddy-for-captions.onrender.com/api/posts/",
        formData,
        {
  ...
  const handleLogout = async () => {
    try {
      await axios.post(
        "https://snapcap-your-buddy-for-captions.onrender.com/api/auth/logout",
        {},
        { withCredentials: true }
      );
    ...
    <div className="min-h-screen bg-gray-50 py-10 px-4">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="text-center mb-6">
          {isAuthLoading ? (
            <span className="px-3 py-1 bg-yellow-100 text-yellow-800 rounded-full">Checking authentication...</span>
          ) : isAuthenticated ? (
            <span className="px-3 py-1 bg-green-100 text-green-800 rounded-full">Logged In</span>
          ) : (
        ...
        <div className="bg-white p-6 rounded-lg shadow-md mb-6">
          ...
          <div className="flex justify-center">
            <label className="flex flex-col items-center justify-center w-full h-64 border-2 border-dashed border-blue-300 rounded-lg cursor-pointer hover:bg-gray-100 bg-gray-50">
              {imagePreview ? (
                ...
                <p className="text-gray-500 text-center mt-20">Click to upload or drag & drop</p>
              )}
          ...
          <div className="flex justify-center mt-4">
            <button
              ...
              disabled={!selectedImage || isLoading || !isAuthenticated}
              className={`px-6 py-2 rounded-lg text-white font-medium ${
                !selectedImage || isLoading || !isAuthenticated ? "bg-gray-300 cursor-not-allowed" : "bg-blue-600 hover:bg-blue-700"
              }`}
            >
